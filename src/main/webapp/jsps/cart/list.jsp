...
<body>
	...
	<c:choose>
		...
		<c:otherwise>
				...
				<c:forEach items="${clist}" var="c">
					<tr align="center">
						...
						<td align="left" width="400px"><a
							href="<c:url value='/bookAction/getBookByBid.do?bid=${c.book.bid}'/>"><span>${c.book.bname }</span></a>
						</td>
						...
						<td><a
							href="<c:url value='/cartItemAction/removeCartItemByCid.do?cartItemId=${c.cartItemId }'/>" onclick="return confirm('确定删除此商品吗？')">删除</a></td>
					</tr>
				</c:forEach>
				<tr>
			...
			<form id="jieSuanForm" action="<c:url value='/cartItemAction/listCartItemsByCids.do'/>"
				method="post">
				<input type="hidden" name="cartItemIds" id="cartItemIds" />
                <input type="hidden" name="total" id="hiddenTotal" />
                <input type="hidden" name="flag" value="listCartItemsByCids" />
			</form>
				...
				$(function() {
									...
									function() {
										...
										if (quantity == 1) {
											if (confirm("您是否真要删除该条目？")) {
												//按照购物车条目表主键删除记录
												$.post(
                                                    "/ssm/cartItemAction/removeCartItemByCidAjax.do",
											...
											$
													.post(
															"/ssm/cartItemAction/minusQuantityByCidAjax.do",
															{
																"cartItemId" : cartItemId
															},
															function() {
					...
					$(".jia")
							.click(
									function() {
										...
										$
												.post(
														"/ssm/cartItemAction/plusQuantityByCidAjax.do",
														{
															"cartItemId" : cartItemId
														}, function() {
															//window.location.reload();
                    ...
                    function() {
                        if (confirm("确定要删除所选项目？")) {
                            ...
                            $.post(
                                "/ssm/cartItemAction/removeCartItemsByCids.do",
                                {"a[]" : cids},
                                function() {
                                    window.location.reload();
                                }, "json");
